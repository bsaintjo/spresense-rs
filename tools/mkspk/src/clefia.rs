...
fn mul4(x: u8) -> u8 { mul2(mul2(x)) }
fn mul6(x: u8) -> u8 { mul2(x) ^ mul4(x) }
fn mul8(x: u8) -> u8 { mul2(mul4(x)) }
fn mula(x: u8) -> u8 { mul2(x) ^ mul8(x) }
...
fn f0_xor(dst: &mut [u8], src: &[u8], rk: &[u8]) {
    ...
    let z = [S0[x[0] as usize], S1[x[1] as usize], S0[x[2] as usize], S1[x[3] as usize]];
    let y = [
    ...
    for i in 0..4 { dst[4 + i] = src[4 + i] ^ y[i]; }
}

// F1 round function (XOR into dst[4..8])
fn f1_xor(dst: &mut [u8], src: &[u8], rk: &[u8]) {
    let x = xor4(&src[..4], &rk[..4]);
    let z = [S1[x[0] as usize], S0[x[1] as usize], S1[x[2] as usize], S0[x[3] as usize]];
    let y = [
...
pub fn keyset(skey: &[u8; 16]) -> (Vec<u8>, usize) {
    ...
    for i in 0..9 {
        ...
        if i % 2 != 0 {
            for j in 0..16 { rk[rk_off + j] ^= skey[j]; }
        }
        double_swap(&mut lk);
        rk_off += 16;
    }
...
pub fn encrypt(pt: &[u8; 16], rk: &[u8], rounds: usize) -> [u8; 16] {
    ...
    for i in 0..4 { rin[4 + i] ^= rk[i]; }
    for i in 0..4 { rin[12 + i] ^= rk[4 + i]; }
    let rout = gfn4(&rin, &rk[8..], rounds);
    let mut ct = rout;
    // final key whitening
    let wk_off = 8 + rounds * 8;
    for i in 0..4 { ct[4 + i] ^= rk[wk_off + i]; }
    for i in 0..4 { ct[12 + i] ^= rk[wk_off + 4 + i]; }
    ct
}
...
impl Cipher {
    ...
    pub fn calc_cmac(&mut self, data: &[u8]) -> Option<[u8; 16]> {
        ...
        let mut p = data;
        while !p.is_empty() {
            for i in 0..16 { m[i] = self.vector[i] ^ p[i]; }
            self.vector = encrypt(&m, &self.rk, rounds);
            p = &p[16..];
        }
        // final: cmac = E(m XOR k1)
        let mut tmp = [0u8; 16];
        for i in 0..16 { tmp[i] = m[i] ^ self.k1[i]; }
...
mod tests {
    ...
    fn encrypt_c_reference() {
        ...
        let expected: [u8; 16] = [
            0xab,0x46,0xc1,0x40,0xbe,0xa1,0xd7,0x70,
            0xda,0x0c,0x87,0x0f,0xe7,0xa2,0x22,0xa4,
        ];
        let (rk, rounds) = keyset(&key);
    ...
    fn cmac_vmk_zeros32() {
        ...
        let expected: [u8; 16] = [
            0xde,0x2b,0x5b,0x8a,0xce,0xfd,0x7d,0x60,
            0x8c,0x5d,0xc8,0x35,0xf9,0xc5,0xd0,0xc5,
        ];
        let data = [0u8; 32];
